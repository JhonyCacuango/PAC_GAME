...
class Gameover():
    ...
    def __init__(self):
        self.ventana = Tk()
        self.ventana.title('GAME OVER ')
        self.ventana.config(bg='dark orange')  
        self.ventana.wm_attributes("-topmost", 1)
   
    
    
    def unjugador(self,nombre,cantidad):
       
        self.nombre_label = Label(self.ventana,text="FIN DEL JUEGO ",bg='light goldenrod')#etiqueta
        self.nombre_label.grid(pady=20,row=2,column=1)
        self.datos=Label(self.ventana,text=nombre+" consiguio "+str(cantidad)+" monedas",bg='light goldenrod')
        self.datos.grid(padx=20,row=3,column=1)
        self.salir = Button(self.ventana,text="REGRESAR AL MENU",width=40,bg='chartreuse3',command=self.ventana.destroy)
        self.salir.grid(padx=20,pady=20,row=4,column=1)
        #self.reintentar = Button(self.ventana,text="REINTENTAR",width=40,bg='salmon2',command= abrirjuego and self.ventana.destroy)
        #self.reintentar.grid(padx=10,pady=10,row=5,column=1)

        self.ventana.mainloop()
    def dosjugador(self,nombre,cantidad,nombre2,cantidad2):            
        self.ventana.config(bg='darkolivegreen3') 
        self.nombre_label = Label(self.ventana,text="FIN DEL JUEGO ",bg='skyblue3')#etiqueta
        self.nombre_label.grid(pady=20,row=2,column=1)
        if cantidad>cantidad2:
            self.datos=Label(self.ventana,text=nombre+" gano y consiguio "+str(cantidad)+" de monedas",bg='light goldenrod')
            self.datos.grid(padx=20,row=3,column=1)
        else:
            self.datos=Label(self.ventana,text=nombre2+" gano y consiguio "+str(cantidad2)+" de monedas",bg='light goldenrod')
            self.datos.grid(padx=20,row=3,column=1)
            
        self.salir = Button(self.ventana,text="REGRESAR AL MENU",width=40,bg='orange2',command= self.ventana.destroy)
        self.salir.grid(padx=20,pady=20,row=4,column=1)
        
        #self.reintentar = Button(self.ventana,text="REINTENTAR",width=40,bg='cyan3',command=abrirmulti and self.ventana.destroy)
        #self.reintentar.grid(padx=10,pady=10,row=5,column=1)
...
def Nivel(nombre):
    ...
    while terminar == False:
        ...
        for event in pygame.event.get():
            ...
            if vidas==0:
                    pygame.mixer.music.load('audiogameover.mp3')
                    pygame.mixer.music.play()
                    fin.unjugador(nombre,cantidad)
                    main()
        ...
        if vidas==5:
            screen.blit(vida,(300,5))
            screen.blit(vida,(340,5))
            screen.blit(vida,(380,5))
            screen.blit(vida,(420,5))
            screen.blit(vida,(460,5))
        elif vidas==4:
            screen.blit(vida,(300,5))
            screen.blit(vida,(340,5))
            screen.blit(vida,(380,5))
...
def Multijugador(nom1,nom2):
    ...
    while game_over == False:
        ...
        contador1=fuente1.render("MONEDAS "+nom1+" :"+ str(total),0,pygame.Color('red'))
        contador2=fuente1.render("MONEDAS "+nom2+" :" +str(total2),0,pygame.Color('yellow'))
        if segundosint>=20:
            terminar=True
            pygame.mixer.music.load('audiogameover.mp3')
            pygame.mixer.music.play()
            fin.dosjugador(nom1, cantidad, nom2, cantidad2)
            main()
        if terminar==False:
...
def main():
    ...
    boton4=Boton(multini,multiplayer,100,250)
        ...
        for event in pygame.event.get():
            ...
            if event.type==pygame.MOUSEBUTTONDOWN:
                ...
                if cursor1.colliderect(boton2.rect):# boton jugar 
                    ...
                    entrada.ingresonombre()
                    Nivel(entrada.nombre())#carga la funcion nivel
                    
                if cursor1.colliderect(boton4.rect):# boton jugador multiplayer
                    pygame.mixer.music.stop()# para el audio
                    entrada.ingressodosjugadores()
                    Multijugador(entrada.nombre1(),entrada.nombre2())#carga la funcion nivel
